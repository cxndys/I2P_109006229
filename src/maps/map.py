...
# ------------------------------
# Bush Layer (for wild Pokémon)
# ------------------------------
        try:
            self.bush_layer = self.tmxdata.get_layer_by_name("PokemonBush")
        except:
            self.bush_layer = None


        self.wild_monsters = [
            {"name": "Pikachu", "hp": 30, "max_hp": 30, "level": 5, "sprite_path": "menu_sprites/menusprite1.png"},
            {"name": "Oddish", "hp": 25, "max_hp": 25, "level": 4, "sprite_path": "menu_sprites/menusprite7.png"},
            {"name": "Bulbasaur", "hp": 35, "max_hp": 35, "level": 6, "sprite_path": "menu_sprites/menusprite4.png"},
        ]
    ...
    def is_bush(self, x, y) -> bool:
        try:
            layer = self.tmxdata.get_layer_by_name("PokemonBush")
        except:
            return False

    # get layer index (pytmx requires this)
        layer_index = self.tmxdata.layers.index(layer)

        props = self.tmxdata.get_tile_properties(x, y, layer_index)
        if props and props.get("type") == "bush":
            return True
    ...
    def from_dict(cls, data: dict) -> "Map":
        ...
        map_obj.shop_npc = data.get("shop_npc", [])
        return map_obj
